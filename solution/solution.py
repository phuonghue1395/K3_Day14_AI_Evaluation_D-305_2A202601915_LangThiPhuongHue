...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        for chunk in contexts:
            ...
            rel = len(chunk_tokens & exp_tokens) / len(exp_tokens)
            relevant_flags.append(1 if rel >= relevance_threshold else 0)
...
class LLMJudge:
    ...
    def score_response(
        ...
        try:
            ...
            for k in rubric.keys():
                scores_dict[k] = float(scores.get(k, 0.5))
        except Exception:
            scores_dict = {k: 0.5 for k in rubric.keys()}
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for item in scores_batch:
            ...
            r1_key = next((k for k in keys if "1" in k or "first" in k.lower()), None)
            r2_key = next((k for k in keys if "2" in k or "second" in k.lower()), None)
            if r1_key and r2_key:
                if scores[r1_key] > scores[r2_key]:
                    r1_higher += 1
                elif scores[r2_key] > scores[r1_key]:
                    r2_higher += 1

        positional_bias = False
        if r1_higher + r2_higher > 0:
            positional_bias = (r1_higher / (r1_higher + r2_higher)) > 0.7
...
class BenchmarkRunner:
    ...
    def run(
        ...
        for pair in qa_pairs:
            actual_ans = agent_fn(pair.question)
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        min_metrics = [k for k, v in scores.items() if v == min_val]
    ...
    def generate_improvement_log(self, failures: list[EvalResult], suggestions: list[str]) -> str:
        ...
        for i, f in enumerate(failures):
            fid = None
            if f.qa_pair and f.qa_pair.metadata:
                fid = f.qa_pair.metadata.get("id")
            if not fid:
                fid = f"F{i+1:03d}"
            ...
            sug = suggestions[i] if i < len(suggestions) else (suggestions[-1] if suggestions else "Review pipeline configuration")
    ...
    def generate_improvement_suggestions(
        ...
        if counts.get("irrelevant", 0) > 0:
            suggestions.append("Improve prompt clarity and focus to keep the model on-topic")
        ...
        default_suggestions = [
            ...
            "Add few-shot examples showing complete answers to improve completeness",
            "Implement hallucination checker to filter unsupported claims",
            "Tune retriever hyperparameters (e.g. top-k chunks) to retrieve more relevant context",
            "Refine system instructions to strictly prevent answering outside of the retrieved context"
        ]
